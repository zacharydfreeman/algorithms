...
class RandomizedSet {
  ...
  insert(val) {
    ...
    this.numbersMap[val] = this.numbers.length - 1;
    return true;
  }
  ...
  remove(val) {
    if (!(val in this.numbersMap)) return false;
    // get index
    const idx = this.numbersMap[val];
    const lastVal = this.numbers[this.numbers.length - 1];
    // swap with last element and pop
    [this.numbers[idx], this.numbers[this.numbers.length - 1]] = [
      this.numbers[this.numbers.length - 1],
      this.numbers[idx],
    ];
    // update number that got move
    this.numbersMap[lastVal] = idx;
    this.numbers.pop();
    delete this.numbersMap[val];
