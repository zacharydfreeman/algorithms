...
const siftDown = (startIdx, minHeap) => {
    ...
    while (childOneIdx < minHeap.length) {
        ...
        let idxToSwap;
        if (childTwoIdx !== -1 && minHeap[childTwoIdx].freq < minHeap[childOneIdx].freq) {
        ...
        if (minHeap[currentIdx].freq > minHeap[idxToSwap].freq) {
            // swap
            [minHeap[currentIdx], minHeap[idxToSwap]] = [minHeap[idxToSwap], minHeap[currentIdx]];
            // update pointers
...
const remove = (minHeap) => {
    ...
    [minHeap[0], minHeap[minHeap.length - 1]] = [minHeap[minHeap.length - 1], minHeap[0]];
    // pop from end
    minHeap.pop();
    // sift down
    siftDown(0, minHeap);
...
const words = ["i","love","leetcode","i","love","coding", "i", "i", "zack", "zack", "zack", "love"]
const k = 3;
